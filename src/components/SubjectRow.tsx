...
export default function SubjectRow({ subject, onUpdate, onDelete }: SubjectRowProps) {
  ...
  return (
    <div className={`flex flex-col gap-2 p-3 rounded-lg border transition-colors
      ${subject.isExcluded ? 'bg-gray-50 border-gray-200' : 'bg-white border-red-50'}`}
    >
      {/* Subject Name — full width on its own row so long names have space */}
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <div className="flex items-center gap-1.5">
          <span className="text-[10px] text-gray-400 uppercase tracking-wider">Units</span>
          <div className="flex items-center border border-red-200 rounded-lg overflow-hidden">
            <button
              onClick={() => handleUnitStep(-1)}
              className="px-2 py-1.5 text-red-500 hover:bg-red-50 active:bg-red-100 text-sm font-bold transition-colors"
            >−</button>
            <input
              type="number"
              min={1}
        ...
        <div className="flex items-center gap-1.5">
          ...
          <div className="flex items-center border border-red-200 rounded-lg overflow-hidden">
            ...
            <select
              ...
              className="px-1 py-1.5 text-sm font-semibold text-red-600 bg-transparent focus:outline-none cursor-pointer"
            >
              {GRADE_OPTIONS.map(g => (
